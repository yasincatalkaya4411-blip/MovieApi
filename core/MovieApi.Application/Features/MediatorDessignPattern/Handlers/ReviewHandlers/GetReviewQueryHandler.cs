...
using MovieApi.Application.Features.MediatorDessignPattern.Queries.ReviewQueries;
...
namespace MovieApi.Application.Features.MediatorDesignPattern.Handlers.ReviewHandlers
{
    public class GetReviewQueryHandler : IRequestHandler<GetReviewQuery, List<GetReviewQueryResult>>
    {
        ...
        {
            var values = await _context.Reviews.Include(x => x.Movies).Skip((request.Page - 1) * request.PageSize).Take(request.PageSize)
